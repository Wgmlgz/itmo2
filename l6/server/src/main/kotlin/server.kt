...
class CmdServer(private val save: String, private val autoSave: String) : Logging {
    ...
    private fun help() =
        io.printer.println(
            commands.filter { it.value.first.isNotEmpty() }.map { it.value.first }.joinToString("\n")
        )
    ...
    private val commands: Map<CommandType, Pair<String, (m: Command) -> Unit>> = mapOf(
        CommandType.Help to Pair("help : output help for available commands") { help() },
        ...
        CommandType.Clear to Pair("clear : clear the collection")
        { q.clear() },
        ...
        CommandType.Exit to Pair("exit : terminate the program (without saving to a file)")
        { io.printer.println("exit on server??? u mad???") },
